...
const LoginComponent = () => {
    ...
    const {isLoading = false, login = {data: null}, error = null} = useSelector(
    ...
    useEffect(() => {
        ...
        if (isMounted && error?.reason == "not_verified") {
            const email = getValues('email');
            router.push({pathname: "/user-verification", query: {email: email}}).then((r) => r);
        }

    }, [login?.time, error?.reason, getValues]);
